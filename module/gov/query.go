...
func QueryProposal(cdc *go_amino.Codec, ip string, pID int64) (result atype.ResultProposal, err error) {
	...
	result = atype.ResultProposal{
		...
		SubmitTime:pr.SubmitTime.Format(time.RFC3339),
		DepositEndTime:pr.DepositEndTime.Format(time.RFC3339),
		TotalDeposit: pr.TotalDeposit,

		VotingStartTime: pr.VotingStartTime.Format(time.RFC3339),
		VotingStartHeight: pr.VotingStartHeight,
		VotingEndTime: pr.DepositEndTime.Format(time.RFC3339),
	}
	return
}

func QueryProposals(cdc *go_amino.Codec, ip string/*limit int64, depositor, voter, statusStr string*/) (result []atype.ResultProposal, err error) {
	cliCtx := context.NewCLIContext().WithCodec(cdc).WithNodeIP(ip)
...
func QueryVotes(cdc *go_amino.Codec, ip string, pID int64) (votes []types.Vote, err error) {
	cliCtx := context.NewCLIContext().WithCodec(cdc).WithNodeIP(ip)
...
func QueryDeposits(cdc *go_amino.Codec, ip string, pID int64) (deposits []types.Deposit, err error) {
	cliCtx := context.NewCLIContext().WithCodec(cdc).WithNodeIP(ip)
...
func QueryTally(cdc *go_amino.Codec, ip string, pID int64 /*, addrStr string*/) (result types.TallyResult, err error) {
	cliCtx := context.NewCLIContext().WithCodec(cdc).WithNodeIP(ip)
